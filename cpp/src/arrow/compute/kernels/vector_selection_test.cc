...
TYPED_TEST(TestTakeKernelWithString, TakeViewMultiplePayloadBuffers) {
  if (!is_binary_view_like(this->value_type()->id())) {
    GTEST_SKIP() << "requires a binary-view-like type";
  }
  auto values = MakeViewArrayWithMultiplePayloadBuffers(this->value_type());
  auto indices = ArrayFromJSON(int32(), "[2, 4, 0, 5]");
  auto expected = ArrayFromJSON(this->value_type(),
                                R"(["first-long-value", "second-long-value", "", "z"])");

  DoCheckTakeAAA(values, indices, expected);

  ASSERT_OK_AND_ASSIGN(Datum actual_datum, CallFunction("array_take", {values, indices}));
  auto actual = actual_datum.make_array();
  ValidateOutput(*actual);
  AssertArraysEqual(*expected, *actual, /*verbose=*/true);
  ...
  auto sliced_expected = ArrayFromJSON(
      this->value_type(), R"(["first-long-value", "second-long-value", "tiny"])");
